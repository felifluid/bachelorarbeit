...
GKW supports both linear and non-linear regimes, for both local and global toroidal simulations. 
This thesis only focuses on linear and non-linear global runs for all purposes.
The code is freely available and currently being hosted at https://bitbucket.org/gkw/gkw. 
For in-depth information about GKW and how it works see #cite(<peeters2015>, form: "prose").
...
A safety factor of $q=5/4$ would mean that the magnetic field line reaches its starting position after exactly one and a fourth toroidal turn.
...
Both plots do not represent the whole torus, but just its outermost mantle ($psi=psi_"max"$).
The grid in @fig:hamada:x:h has been adjusted to emphasize the relationship of $zeta$ and $q$.
The top line is $zeta(s, phi=0)$, while the bottom line represents $zeta(s, phi=2pi)$ with all other toroidal angles lying in between.
...
The discontinuity of the $zeta$\-grid at $s=±0.5$ is clearly visible.
Also note that the s-grid is more dense on the left side (inner radius) than on the right side (outer radius).
This is caused by a discrepancy between the regularly spaced s-grid and the non-uniform geometry of the torus. // passt das??
...
This can yet again be generalized for any function
...
The change in $theta$ translates to a change in $s$ from $s_A = 0$ to $s_B = ±1$.
...
In this $n_"min"$ denotes the fraction of the torus data that was simulated @samaniego2024topovis[p.19].
...
This is caused by preferring tiny, delaunay-conform triangles in high-density ares, instead of big acute triangles in low-density areas. 
...
Both offer algorithmic approaches to add the least amount of extra vertices to the grid so that no resulting angles exceed a given angle. 
_Triangle_ is an implementation of this and is freely available at https://www.cs.cmu.edu/~quake/triangle.html @shewchuck2025triangle.]. 
...
== Interpolation
This section focuses on interpolation of multidimensional data, as the data being interpolated in ToPoVis is multidimensional.
...
With each geometry having its own definition of $G$. 

==== Circular Geometry
// #sym.psi\(r) &= frac(r, R_#text("ref")) \
// s(r, #sym.theta) &= frac(1, 2#sym.pi) [#sym.theta + #sym.psi\(r) sin(#sym.theta)] \
In circular geometry, the factor $G$ is defined as follows @peeters2015[p.25].
...
Each dataset is referenced by the key `Poten` followed by an 8 digit long, e.g. `Poten00000343`. 
...
Oftentimes it is more useful to look at the non-zonal potential, as it reveals more about the turbulence structures. // TODO: Quelle?
...
Either one can be chosen interactively, when running ToPoVis on non-linear simulation data.
However, B-Spline interpolation is the prefered option, as it yielded better results in benchmarking @samaniego2024topovis[p.29ff].
